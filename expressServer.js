...
app.get("/login", (req, res) => {
	const templateVars = {
		sentence:
			"Before using TinyURL, please sign-in. If you do not have an account, ",
		redirect: "register",
		page: "login"
	};
...
app.get("/register", (req, res) => {
	const templateVars = {
		sentence:
			"If you do not have an account, please register. If you are already registered, ",
		redirect: "login",
		page: "register"
	};
	res.render("welcome", templateVars);
...
app.post("/:user/new", (req, res) => {
  ...
  URLDatabase[req.params.user][shortURL] = req.body.longURL;
  analytics[shortURL] = {visits: 0, uniqueVisits: 0}
...
app.delete("/:user/urls/:shortURL", (req, res) => {
	delete URLDatabase[req.params.user][req.params.shortURL];
	res.redirect(`/${req.params.user}/urls`);
});

app.get("/:user/urls/:shortURL/edit", (req, res) => {
	res.render("editURL", {
...
app.put("/:user/urls/:shortURL", (req, res) => {
	const longURL = URLDatabase[req.params.user][req.params.shortURL];
	delete URLDatabase[req.params.user][req.params.shortURL];
	URLDatabase[req.params.user][req.body.shortURL] = longURL;
	res.redirect(`/${req.params.user}/urls`);
});
